...
[개념]
시작노드에서 출발 -> 노드 선택 -> 최대 깊이까지 탐색 후 부모 노드로 재귀 -> 다른 쪽 노드 출현 시, 그쪽으로 이동하여 다시 깊이 우선 탐색 -> ...
- entry time: 깊이 탐색 시 stack에 추가된 순서 + neighbor node가 없는 node 방문 시, stack에서 빠져나오면서 부여되는 순서
기능: 그래프 완전 탐색
특징: 재귀 함수로 구현(recursion tree), 스택 자료구조(FILO)이용
